...
function ValueSpan({ value }: { value: unknown }) {
  ...
  if (typeof value === 'boolean') return <span className="tok-boolean">{String(value)}</span>
...
function TreeNode({ value, keyName, depth }: TreeNodeProps) {
  ...
  const keyEl = keyName !== undefined
    ? <><span className="tree__key tok-key">"{keyName}"</span><span className="tree__colon">: </span></>
    : null
  ...
  return (
    <div className="tree-node" style={nodeStyle}>
      ...
      >
        <span className={`tree__arrow${collapsed ? '' : ' tree__arrow--open'}`} aria-hidden="true">▶</span>
        {keyEl}
